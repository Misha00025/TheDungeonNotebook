...
Принимает в теле запроса одно или несколько полей из базовой структуры [`character`](#персонаж-character). Игнорирует дополнительные поля и поле `id`. Заменяет поля существующего персонажа полями из переданных данных.