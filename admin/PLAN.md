...
| 14 | Добавление пользователя в группу | Форма на странице деталей группы (`/admin/groups/<id>/add-user` POST). Использует `PUT /polices/groups` с `isAdmin=false`. Поле для ввода User ID и кнопка "Add to Group". | ✓ |
