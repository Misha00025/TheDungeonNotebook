> **⚠️ ВНИМАНИЕ: Этот сервис устарел.**
> Начиная с рефакторинга, функциональность `policy-service` перенесена в `campaign-service`.
> Политики доступа теперь обслуживаются `campaign-service` на тех же URL `/polices/groups`.
> Исходный код `policy-service` больше не существует.
> Данный файл оставлен для исторической справки.
