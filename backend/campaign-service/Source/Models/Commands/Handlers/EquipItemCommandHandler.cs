...
public class EquipItemCommandHandler : CommandHandler<EquipItemCommand>
{
    ...
    public EquipItemCommandHandler(CommandsProvider provider, CharacterLogProvider log, SubjectAccessHelper access)
    {
        _provider = provider;
        _log = log;
        _access = access;
    ...
    public override CommandResult Execute(int groupId, int characterId, EquipItemCommand command)
    {
        var result = _provider.EquipItem(groupId, characterId, command);
