...
public class AddFieldCommandHandler : ICommandHandler
{
    ...
    public CharacterCommandType Handles => CharacterCommandType.AddField;

    public CommandResult Execute(int groupId, int characterId, CharacterCommandRequest request)
    {
        var result = _provider.AddField(groupId, characterId, request.Payload ?? new CommandPayload());
