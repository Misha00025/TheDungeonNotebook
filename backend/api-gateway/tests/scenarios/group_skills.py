...
def register_group_skills_scenario():
    ...
    tests.append(Test(headers={**h, "Authorization": "{at}"},
        request="groups/{steps.0.id}/skills/{steps.4.id}", method="GET", requirement=OK))

    # 6. PUT /groups/{id}/skills/{skillId} (admin) → 200
    tests.append(Test(headers={**h, "Authorization": "{at}"},
        request="groups/{steps.0.id}/skills/{steps.4.id}", method="PUT",
        data={"name": "Athletics (Improved)"}, requirement=OK))

    # 7. DELETE /groups/{id}/skills/{skillId} (admin) → 200
    tests.append(Test(headers={**h, "Authorization": "{at}"},
        request="groups/{steps.0.id}/skills/{steps.4.id}", method="DELETE", requirement=OK))
